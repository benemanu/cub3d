...
/*   Updated: 2023/03/16 13:32:30 by shoffman         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_load_img_ammo_full(t_info *info, char *img)
{
	info->game.ammo_full.img = mlx_xpm_file_to_image(info->mlx, img,
			&info->game.ammo_full.dimensions.sprite_width,
			&info->game.ammo_full.dimensions.sprite_height);
	info->game.ammo_full.data = mlx_get_data_addr(info->game.ammo_full.img,
			&info->game.ammo_full.dimensions.s_bpp,
			&info->game.ammo_full.size_l,
...
void	ft_load_img_ammo_empty(t_info *info, char *img)
{
	info->game.ammo_empty.img = mlx_xpm_file_to_image(info->mlx, img,
			&info->game.ammo_empty.dimensions.sprite_width,
			&info->game.ammo_empty.dimensions.sprite_height);
	info->game.ammo_empty.data = mlx_get_data_addr(info->game.ammo_empty.img,
			&info->game.ammo_empty.dimensions.s_bpp,
			&info->game.ammo_empty.size_l,
...
void	ft_draw_ammo(t_info *info)
{
	...
	{
		if (tmp > 0)
			ft_draw_sprite(info, info->game.ammo_full.data,
				info->game.ammo_full.size_l,
				&info->game.ammo_full.dimensions);
		else
			ft_draw_sprite(info, info->game.ammo_empty.data,
				info->game.ammo_empty.size_l,
